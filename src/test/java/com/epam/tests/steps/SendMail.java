...
public class SendMail extends BaseTest {
    ...
    public void sendEmail(String subject) {
        Assert.assertTrue(new DraftsPage(DriverProvider.getDriver()).sendDraft(subject).openSentMailPage().isEmailSent(subject), "Email not sent!");
    ...
    public void verifyDraftDeleted(String subject) {
        Assert.assertFalse(new SentPage(DriverProvider.getDriver()).openDraftsPage().isDraftPresent(subject), "Draft not deleted!");
